...
TEST_F(TspEXACT, Quatar194EXACT) {
    testTspExact("../data/qa194.tsp",194,12237);
}

TEST_F(TspEXACT, Djbuti38EXACT) {
    testTspExact("../data/dj38.tsp",38,8718);
}

TEST_F(TspEXACT, Luxemburg980EXACT) {
    testTspExact("../data/lu980.tsp",980,15232);
}

TEST_F(TspEXACT, Uruguay734EXACT) {
    testTspExact("../data/uy734.tsp",734,108140);
}
TEST_F(TspEXACT, Zimbabue929EXACT) {
    testTspExact("../data/zi929.tsp",929,130424);
}
...
/** 
 * values before the bug on coordinate X was fixed 
 * delete if the next class runs ok
TEST_F(TspEXACT, Quatar194EXACT) {
